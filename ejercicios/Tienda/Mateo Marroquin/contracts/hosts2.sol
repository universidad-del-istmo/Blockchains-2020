...
contract hosts2{
    ...
    event dominioRegistrado(string dominio, address owner);
    ...
    function setHost(address payable _address, string memory _ip, string memory _dominio) public {
        ...
        emit dominioRegistrado(_dominio, _address);
    ...
    function getIp(address ins) view public returns (string memory) {
        return (boughtHosts[ins].ip);
    }
    
    function retIp(string memory _dominio) view public returns (string memory){
        address owner = reguladora;
        for (uint i=0; i<boughtHostAccts.length; i++) {
            
            if (keccak256(abi.encodePacked(boughtHosts[boughtHostAccts[i]].dominio)) == keccak256(abi.encodePacked(_dominio))){
                return getIp(boughtHostAccts[i]);
            }
        }
        return "";
    }
